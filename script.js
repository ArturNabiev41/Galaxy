...
for (let i = 0; i < item_bio.length; i++) {
    item_descriptions[i].innerHTML = item_bio[i];
    item_prices[i].innerHTML = item_price[i];
}
...
function addToCart() {
    ...
    let currentAmount = parseInt(cartCounter.textContent);
    cartCounter.innerHTML = currentAmount + 1;
}
